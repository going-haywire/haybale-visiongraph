...
`visiongraph:node:OakDCameraNode` · kind: node
...
| callbacks | inlet | core:type:PooledType | Connect to a Frame Event node |
| status | config | builtin:type:STRING | Text data |
| visiongraph.node.OakDCameraNode.device.mxid | config | builtin:type:STRING | Leave empty to auto-select the first available OAK device. |
