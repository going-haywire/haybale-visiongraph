...
`visiongraph:node:AnnotateNode` · kind: node
...
| result | inlet | core:type:PooledType | Connect one or more estimator result outlets |
...
| show_info | config | builtin:type:BOOL | True or False |
| frame_ready | outlet | core:type:EXEC | Signal for controlling execution flow between nodes |
| annotated | outlet | visiongraph:type:RGB_FRAME | 3-channel uint8 colour video frame |
